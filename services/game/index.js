...
class GameService {
  async createBotGame(id, type, item) {
    ...
    const game = new Game({
      ...
      createdAt: new Date(),
      moves:
        type === 'X' ? this.getMove(item, type) : await this.getBotFirstMove(),
    });
  ...
  isWinner(moves, figure) {
    ...
    for (const combination of winningCombinations) {
      ...
      if (
        moves.some(
          (move) =>
            move.col === a[0] && move.row === a[1] && move.figure === figure,
        ) &&
        moves.some(
          (move) =>
            move.col === b[0] && move.row === b[1] && move.figure === figure,
        ) &&
        moves.some(
          (move) =>
            move.col === c[0] && move.row === c[1] && move.figure === figure,
        )
      ) {
        return true;
      }
  ...
  async getMoves(id, innerRow, innerCol) {
    ...
    const isGameWinner = this.isGameWinner(
      moves,
      botFigure === 'X' ? '0' : 'X',
    );
    ...
    let movesCurrentPoint = moves
      .filter((item) => item.row === innerRow && item.col === innerCol)
      .map((el) => {
        if (el.innerRow !== undefined && el.innerCol !== undefined) {
          return {
            row: el.innerRow,
    ...
    if (movesCurrentPoint.length === 9) {
      ...
      for (let i = 0; i < innerItems.length; i++) {
        ...
        if (candidateMoves.length < 9) {
          ...
          movesCurrentPoint = candidateMoves
            .map((el) => {
              if (el.innerRow !== undefined && el.innerCol !== undefined) {
                return {
                  row: el.innerRow,
  ...
  getTextFilledMove(filledMoves, row, col) {
    const filledItem = filledMoves.find(
  ...
  getTextFilledMoveHuman(filledMoves, row, col) {
    const filledItem = filledMoves.find(
      (item) => item.row === row && item.col === col,
    );
    return filledItem ? filledItem.figure : ' ';
  ...
  getCallbackDataFilledMoveHuman(filledMoves, row, col, index) {
    return !filledMoves.find((item) => item.row === row && item.col === col)
      ? `gameBoardHuman${index}`
      : 'emptyBtn';
  }

  async getFilledMovesHuman(id, innerRow, innerCol) {
    const game = await Game.findOne({
      $or: [{ firstPlayer: id }, { secondPlayer: id }],
      status: 'started',
      gameType: 'human',
  ...
  async getFilledReplyMarkupHuman(id, nextRow, nextCol) {
    const filledMoves = await this.getFilledMovesHuman(id, nextRow, nextCol);

    return {
      inline_keyboard: [
        [
          {
            text: this.getTextFilledMoveHuman(filledMoves, 0, 0),
            callback_data: this.getCallbackDataFilledMoveHuman(
              filledMoves,
              0,
              0,
              1,
          ...
          {
            text: this.getTextFilledMoveHuman(filledMoves, 0, 1),
            callback_data: this.getCallbackDataFilledMoveHuman(
              filledMoves,
              0,
              1,
          ...
          {
            text: this.getTextFilledMoveHuman(filledMoves, 0, 2),
            callback_data: this.getCallbackDataFilledMoveHuman(
              filledMoves,
              0,
              2,
        ...
        [
          {
            text: this.getTextFilledMoveHuman(filledMoves, 1, 0),
            callback_data: this.getCallbackDataFilledMoveHuman(
              filledMoves,
              1,
              0,
          ...
          {
            text: this.getTextFilledMoveHuman(filledMoves, 1, 1),
            callback_data: this.getCallbackDataFilledMoveHuman(
              filledMoves,
              1,
              1,
          ...
          {
            text: this.getTextFilledMoveHuman(filledMoves, 1, 2),
            callback_data: this.getCallbackDataFilledMoveHuman(
              filledMoves,
              1,
              2,
        ...
        [
          {
            text: this.getTextFilledMoveHuman(filledMoves, 2, 0),
            callback_data: this.getCallbackDataFilledMoveHuman(
              filledMoves,
              2,
              0,
          ...
          {
            text: this.getTextFilledMoveHuman(filledMoves, 2, 1),
            callback_data: this.getCallbackDataFilledMoveHuman(
              filledMoves,
              2,
              1,
          ...
          {
            text: this.getTextFilledMoveHuman(filledMoves, 2, 2),
            callback_data: this.getCallbackDataFilledMoveHuman(
              filledMoves,
              2,
              2,
