...
bot.hears('Почати гру', async (ctx) => {
    ...
    if(isBot){
        ctx.reply('Виберіть чим будете грати.', {reply_markup: {remove_keyboard: true}})
...
bot.action("human", async (ctx) => {
    ...
    return ctx.reply('Ви вибрали грати с компьютером', { reply_markup: constants.startGameKeyboard });
});
...
bot.action("bot", async (ctx) => {
    ...
    return ctx.reply('Ви вибрали грати з ботом', { reply_markup: constants.startGameKeyboard });
});
...
bot.action("zero", async (ctx) => {
    ...
    const canvas = CanvasService.createDefaultCanvas(moves, {row: moves[0].innerRow, col: moves[0].innerCol}, moves[0])
...
bot.action('gameBoardbtn1', async (ctx) => {
    ...
    const {moves, nextCol, nextRow, message} = await GameService.getMoves(id, innerRow, innerCol)
    ...
    if(message){
        if(message === 'Виберіть інше поле') {
            ...
            const canvas = CanvasService.createDefaultCanvas(moves, {row: nextRow, col: nextCol}, myMove)
            ...
            await ctx.reply(message.split(',')[1], {reply_markup: constants.startGameKeyboard})
    ...
    const reply_markup = await GameService.getFilledReplyMarkup(id, nextRow, nextCol)

    const canvas = CanvasService.createDefaultCanvas(moves, {row: nextRow, col: nextCol}, botMove)
...
bot.action('chooseAgain1', async (ctx) => {
    ...
    const canvas = CanvasService.createDefaultCanvas(moves, {row, col}, botMove)
